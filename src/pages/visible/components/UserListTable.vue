...
const handleModifyShow = (userConnectUuid: any, deletable: any) => {
  ...
  modifyForm.deletable = deletable;
...
<template>
  ...
  >
    <template #columns>
      ...
      <TableColumn title="导出器地址" data-index="exportIp" />
      <TableColumn title="导出器运行端口" data-index="exportPort" />
      <TableColumn title="是否可见">
        <template #cell="{ record }">
          <Tag v-if="record.visible === 1" color="green"><span>是</span></Tag>
          <Tag v-else-if="record.visible === 0" color="red"><span>否</span></Tag>
        </template>
      </TableColumn>
      <TableColumn title="是否可删">
        <template #cell="{ record }">
          <Tag v-if="record.deletable === 1" color="green"><span>是</span></Tag>
          <Tag v-else-if="record.deletable === 0" color="red"><span>否</span></Tag>
        </template>
      </TableColumn>
