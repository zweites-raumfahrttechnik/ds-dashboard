...
const emit = defineEmits<{
  ...
  (e: 'get-children', num: Record<string, unknown>): void;
}>();

const { data, execute } = useAxios<ResponseWrap<unknown>>(
  REDIS_OP_URL,
  { method: 'POST' },
  instance,
  {
...
const handleSubmit = async () => {
  ...
  execute({
    ...
    emit('get-children', data.value as any);
  });
};
        ...
        <Col :span="8">
          ...
          >
            <Select v-model="form.keyType" placeholder="请选择操作值类型" allow-search>
              <Option v-if="isSetGet" value="item">item</Option>
              <Option v-if="isDelExist" value="string">string</Option>
              <Option value="set">set</Option>
